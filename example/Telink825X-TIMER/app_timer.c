...
void app_timer_test_init(void)
{
	...
	reg_tmr0_capt = 1 * CLOCK_SYS_CLOCK_1S;
...
_attribute_ram_code_ void app_timer_test_irq_proc(void){
	if(reg_tmr_sta & FLD_TMR_STA_TMR0){
		...
		gpio_write(GPIO_PC1, led_status);
		gpio_write(GPIO_PB5, led_status);
		gpio_write(GPIO_PC4, led_status);
		gpio_write(GPIO_PD2, led_status);
		gpio_write(GPIO_PD3, led_status);
		DBG_CHN0_TOGGLE;
	}
