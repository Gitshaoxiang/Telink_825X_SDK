...
const u8 tbl_advData[] = {
    0x0b, 0x09, 'H', 'I', 'D', '-', 'S', 'E', 'L', 'F', 'I', 'E', 0x02, 0x01,
    0x05,                    // BLE limited discoverable mode
...
void task_connect(u8 e, u8 *p, int n) {
    bls_l2cap_requestConnParamUpdate(
        8, 8, 99, 400);  // interval=10ms latency=99 timeout=4s
...
void blt_pm_proc(void) {
    ...
    if (ui_mic_enable) {
        ...
        bls_pm_setSuspendMask(
            SUSPEND_DISABLE);  // phy test can not enter suspend
    }
#endif
    else {
        ...
        int user_task_flg = ota_is_working || scan_pin_need ||
                            key_not_released || DEVICE_LED_BUSY;
        ...
        if (user_task_flg) {
            ...
            if (!ota_is_working && key_matrix_same_as_last_cnt >
                                       5) {  // key matrix stable can optize
        ...
        if (sendTerminate_before_enterDeep ==
            ...
            if (user_task_flg) {  // detect key Press again,  can not enter deep
                                  // now
        ...
        if (!blc_ll_isControllerEventPending()) {  // no controller event
                                                   // pending
            // adv 60s, deepsleep
            if (blc_ll_getCurrentState() == BLS_LINK_STATE_ADV &&
                ...
                cpu_sleep_wakeup(DEEPSLEEP_MODE, PM_WAKEUP_PAD,
                                 0);  // deepsleep
            }
            // conn 60s no event(key/voice/led), enter deepsleep
                ...
                bls_ll_terminateConnection(
                    HCI_ERR_REMOTE_USER_TERM_CONN);  // push terminate cmd into
                                                     // ble TX buffer
...
void key_init(void) {
    ...
    gpio_setup_up_down_resistor(GPIO_PB7, PM_PIN_PULLUP_10K);
...
/*****************************************************************************************
 Note: battery check must do before any flash write/erase operation, cause flash
write/erase under a low or unstable power supply will lead to error flash
operation

       Some module initialization may involve flash write/erase, include: OTA
initialization, SMP initialization, .. So these initialization must be done
after  battery check
*****************************************************************************************/
...
// Smp Initialization may involve flash write/erase(when one sector stores too
// much information,
//    is about to exceed the sector threshold, this sector must be erased, and
//    all useful information should re_stored) , so it must be done after
//    battery check
...
//     //  reConnect:  send security_request 1000mS after connection complete. If
//     //  master start paring or encryption before 1000mS timeout, slave do not
//     //  send security_request.
//     blc_smp_configSecurityRequestSending(
//         SecReq_IMM_SEND, SecReq_PEND_SEND,
//         1000);  // if not set, default is:  send "security request" immediately
//                 // after link layer connection established(regardless of new
...
void main_loop(void) {
    ...
    // u16 consumer_key = 0xe9;

    // if (gpio_read(GPIO_PB7) == 0) {
    //     sleep_ms(20);
    //     if (gpio_read(GPIO_PB7) == 0) {
    //         while (gpio_read(GPIO_PB7) == 0) {
    //             sleep_ms(1);
    //         };
    //         u16 consumer_key = 0xea;
    //         bls_att_pushNotifyData(HID_CONSUME_REPORT_INPUT_DP_H,
    //                                (u8 *)&consumer_key, 2);
    //         sleep_ms(50);
    //     }
    // } else {
    //     u16 consumer_key = 0x00;
    //     bls_att_pushNotifyData(HID_CONSUME_REPORT_INPUT_DP_H,
    //                            (u8 *)&consumer_key, 2);
    // }

    // if (gpio_read(GPIO_PB6) == 0) {
    //     sleep_ms(20);
    //     if (gpio_read(GPIO_PB6) == 0) {
    //         while (gpio_read(GPIO_PB6) == 0) {
    //             sleep_ms(1);
    //         };
    //         u16 consumer_key = 0xe9;
    //         bls_att_pushNotifyData(HID_CONSUME_REPORT_INPUT_DP_H,
    ...
    // if (my_batVal[0] < 100) {
    //     my_batVal[0]++;
	// 	bls_att_pushNotifyData(BATT_LEVEL_INPUT_DP_H,
    //                            my_batVal, 1);
    // } else {
    //     my_batVal[0] = 0;
	// 	bls_att_pushNotifyData(BATT_LEVEL_INPUT_DP_H,
    ...
    // sleep_ms(100);
    // u16 consumer_key = 0xea;
    // bls_att_pushNotifyData (HID_CONSUME_REPORT_INPUT_DP_H, (u8
    // *)&consumer_key, 2);
